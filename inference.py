...
def init(args):
	...
	# print("dictionary:", dictionary) dictionary: <kosmos2_5.tasks.data.dictionary.Dictionary object at 0x7cc24fdd1d90>
...
def build_data(args, image_path, image_processor, dictionary):
	'''
		token = [bos_id] + ([boi_id] * image_token_legth + 1 ) + [eoi_id] + text_tokens
		img_gpt_input_mask = [0] + [0] + [1] * (image_token_legth) + [0] + [0] * text_length  
	'''

	image_token_legth = 2048
	bos_id = dictionary.bos_index
	eos_id = dictionary.eos_index
	boi_id = dictionary.index(BOI_SYMBOL)
	eoi_id = dictionary.index(EOI_SYMBOL)
	...
	img_res = image_processor(images=image, return_tensors="pt", max_patches=2048)
	...
	token.extend([boi_id] * (image_token_legth +1) + [eoi_id])
	img_gpt_input_mask.extend([0] + [1] * image_token_legth + [0])
	segment_token.extend([1] + [1] * image_token_legth + [1])
	if args.do_ocr == True:
		# text_token = [dictionary.index("<ocr>"), dictionary.index('<bbox>')]
		text_token = [dictionary.index(OCR_SYMBOL), dictionary.index(SOB_SYMBOL)]
	else:
	...
	# print("img_src_token", img_src_token.shape) torch.Size([2048, 770])
...
def save_to_txt(translations, tokens, file_name):
    with open(file_name, 'w', encoding='utf-8') as file:
        # 将translations写入文件
        file.write("Translations:\n")
        file.write(str(translations))
        file.write("\n\n")  # 添加空行以区分不同部分
...
def main():
	...
	for i in tqdm(range(len(image_list)), desc='inference'):
		...
		save_to_txt(translations, tokens, "output.txt")
