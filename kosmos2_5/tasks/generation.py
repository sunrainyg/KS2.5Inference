...
from kosmos2_5.data.utils import EOL_SYMBOL, BOIMG_SYMBOL, EOIMG_SYMBOL, SPECIAL_SYMBOLS
...
class GenerationTask(LanguageModelingTask):
    ...
    def setup_dictionary(cls, args, **kwargs):
        ...
        if len(args.dict_path) > 0:
            ...
            # dictionary = Dictionary.load(args.dict_path, from_SPM=True)
        else:
            # dictionary = Dictionary.load(os.path.join(paths[0], "dict.txt"))
            print("dict path is none")
        # dictionary.add_symbol(EOL_SYMBOL)
        # dictionary.add_symbol(BOIMG_SYMBOL)
        # dictionary.add_symbol(EOIMG_SYMBOL)
        for special_symbol in SPECIAL_SYMBOLS:
            dictionary.add_symbol(special_symbol)
            
        dictionary.pad_to_multiple_(args.required_batch_size_multiple)
        for img_codebook_index in range(8192): # image_codebook_size': 8192
            dictionary.add_symbol(f"<img_{img_codebook_index}>")
